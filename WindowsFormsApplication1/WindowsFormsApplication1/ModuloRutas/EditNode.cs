...
namespace Dijkstra
{
    public partial class EditNode : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("El nombre del nodo está en blanco! ", " Error al editar nodo", MessageBoxButtons.OK,
                ...
                if (Convert.ToBoolean(row.Cells[0].Value))
                {
                    ...
                    if (!int.TryParse(row.Cells[2].Value.ToString(), out weight))
                    {
                        MessageBox.Show("La distancia de arista del nodo \"" + row.Cells[1].Value.ToString() + "\" no es un valor numérico válido!",
                            "Error al editar nodo", MessageBoxButtons.OK,
                            MessageBoxIcon.Error);
                        return;
                    }
                    if (weight < 0)
            ...
            foreach (DataGridViewRow row in this.dataGridView1.Rows)
            {
                ...
                // AQUI AÑDIR EL UPDATE A LA BASE DE DATOS.
            }
